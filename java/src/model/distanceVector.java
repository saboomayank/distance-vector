...
public class distanceVector {
    ...
    public static void update(int serverId1, int serverId2, int cost) throws IOException {
        if(serverId1 == myID){
            ...
            if(isNeighbor(to)){
                routingTable.put(to, cost);
                Message message = new Message(myNode.getId(),myNode.getIp(),myNode.getPort(),"update");
                message.setRoutingTable(makeMessage());
                sendMessage(to,message);
                System.out.println("Message sent to "+to.getIp());
    ...
    public static void step() throws IOException{
        if(neighbors.size()>=1){
            Message message = new Message(myNode.getId(),myNode.getIp(),myNode.getPort(),"step");
            message.setRoutingTable(makeMessage());
            for(Node eachNeighbor:neighbors) {
                sendMessage(eachNeighbor,message); //sending message to each neighbor
                System.out.println("Message sent to "+eachNeighbor.getIp()+"!");
            }
        ...
        try {
            ...
            if(semaphore>0) {
                ...
                while(selectedKeysIterator.hasNext())
                {
                    ...
                    if(parseChannelIp((SocketChannel)selectionKey.channel()).equals(eachNeighbor.getIp()))
                    {
    ...
    public static boolean disable(Node server) throws IOException{
        if(isNeighbor(server)){
            ...
            sendMessage(server,new Message(myNode.getId(),myNode.getIp(),myNode.getPort(),"disable"));
            for(SocketChannel channel:openChannels){
                if(server.getIp().equals(parseChannelIp(channel))){
                    try {
                        channel.close();
                    } catch (IOException e) {
            ...
            System.out.println("Disabled connection with server "+server.getId()+"("+server.getIp()+")");
            return true;
        }
        else{
